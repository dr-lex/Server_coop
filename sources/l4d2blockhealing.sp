...
#define PLUGIN_VERSION                  "1.1.4"                             // Version of the plugin
...
ConVar g_hBlockMedkitTime_Cvar;
ConVar g_hBlockMedkitTime_Ladder_Cvar;
ConVar g_hBlockUseTime_Cvar;
...
public void OnPluginStart()
{
    ...
    g_hBlockMedkitTime_Cvar = CreateConVar(cvarName, "5.0", "How long the healing Survivor is prohibit from taking out their medkit after the receiving Survivor breaks free of the heal. 0 to disable prohibition", FCVAR_NONE);

    Format(cvarName, sizeof(cvarName), "%s_time_ladder", PLUGIN_CVAR_PREFIX);
    g_hBlockMedkitTime_Ladder_Cvar = CreateConVar(cvarName, "15.0", "How long the healing Survivor is prohibit from taking out their medkit after trying to heal a fellow Survivor on a ladder. 0 to disable prohibition", FCVAR_NONE);

    Format(cvarName, sizeof(cvarName), "%s_time_use", PLUGIN_CVAR_PREFIX);
    g_hBlockUseTime_Cvar = CreateConVar(cvarName, "5.0", "How long the reviving Survivor is prohibit from using their use button after the incapacitated Survivor breaks free of the reviving. 0 to disable Survivors being able to break free of a revive", FCVAR_NONE);
