...
// Cvar handles
ConVar cvar_MinTime = null;
ConVar cvar_MaxPing = null;
ConVar cvar_CheckRate = null;
ConVar cvar_MaxWarnings = null;
ConVar cvar_MinPlayers = null;
ConVar cvar_KickMsg = null;
ConVar cvar_LogActions = null;
ConVar cvar_ShowPublicKick = null;
ConVar cvar_KickMsgPublic = null;
ConVar cvar_ShowWarnings = null;
ConVar cvar_WarningMsg = null;
ConVar cvar_ImmunityFlag = null;
...
public void OnPluginStart()
{
	...
	cvar_MaxPing = CreateConVar("sm_vbping_maxping",			"400",				"Maximum player ping", FCVAR_NONE, true, 1.0);
	cvar_CheckRate = CreateConVar("sm_vbping_checkrate",			"20.0",				"Period in seconds when rate is checked", FCVAR_NONE, true, 1.0);
	cvar_MaxWarnings = CreateConVar("sm_vbping_maxwarnings",		"10",				"Number of warnings before kick", FCVAR_NONE, true, 1.0);
	cvar_MinPlayers = CreateConVar("sm_vbping_minplayers",			"3",				"Minimum number of players before kicking", FCVAR_NONE);
	cvar_LogActions = CreateConVar("sm_vbping_logactions",			"1",				"Log warning and kick actions. 0 = Disabled, 1 = Enabled", FCVAR_NONE, true, 0.0, true, 1.0);
	...
	cvar_KickMsg = CreateConVar("sm_vbping_kickmsg",			"Your ping is too high",				"Kick message sent to kicked client", FCVAR_NONE);
	...
	cvar_KickMsgPublic = CreateConVar("sm_vbping_kickmsgpublic",	        "{NAME} Your ping is too high",				"Public kick message. {NAME} converts to the player's name.", FCVAR_NONE);
...
public Action cmd_Debug(int client, int args)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientConnected(i) && IsClientInGame(i) && !IsFakeClient(i))
		{
			...
			if ((GetUserFlagBits(i) & ReadFlagString(Flag)) || (GetUserFlagBits(i) & ADMFLAG_ROOT)) PrintToConsole(client, " . %-32N Ping: %-8.0f *IMMUNE*", i, Ping);
			else if (PingDelay[i]) PrintToConsole(client, " . %-32N Ping: %-8.0f *CONNECTING*", i, Ping);
			else PrintToConsole(client, " . %-32N Ping: %-8.0f Warnings: %i", i, Ping, PingWarnings[i]);
		}
	}
...
public int action_RateChanged(Handle convar, const char[] oldValue, const char[] newValue)
{
...
public Action timer_CheckPing(Handle timer)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientConnected(i) && IsClientInGame(i) && !IsFakeClient(i)) Players++;
	}
