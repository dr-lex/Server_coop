...
#define FOR_EACH_UNCOMMON_TYPE(%1) for(int %1 = 0; %1 < sizeof(UncommonData); %1++)
...
int RemainingZombiesToSpawn = 0;
int HordeNumber = 0;
ConVar HordeAmountCVAR = null;
ConVar RandomizeUCI = null;
ConVar RandomizeUCIChance = null;
ConVar AllowedUCIFlags = null;
ConVar UCIHealthOverride = null;
bool AutoShuffleEnabled = false;
bool AreModelsCached = false;
int UncommonInfectedChance = 0;
int AllowedUCIFlag = 0;
int UCIHealthOverrideValue = -1;

int UncommonData[Uncommons][UncommonInfo];
...
public void OnPluginStart()
{
	...
	UCIHealthOverride = CreateConVar("l4d2_spawn_uncommons_healthoverride", "1000", "Health value the uncommons get set to. '-1' is default values", FCVAR_NONE | FCVAR_NOTIFY);
...
public void ConvarsChanged(Handle convar, const char[] oldValue, const char[] newValue)
{
	AutoShuffleEnabled = GetConVarBool(RandomizeUCI);
	UncommonInfectedChance = GetConVarInt(RandomizeUCIChance);
	AllowedUCIFlag = GetConVarInt(AllowedUCIFlags);
...
public Action Command_UncommonHorde(int client, int args)
{
	...
	CheatCommand(GetAnyClient(), "z_spawn_old", "mob");
...
{
	if (!AreModelsCached || !StrEqual(classname, "infected", false))
	{
		return;
	}
...
public Action SpawnUncommonInf(int number, float location[3])
{
...
static int HandleHealthOverride(int entity)
{
...
int CheatCommand(int client, const char[] command, const char[] arguments = "")
{
	if (!client) return;
