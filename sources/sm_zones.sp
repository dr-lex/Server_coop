...
// ====[ VARIABLES ]=========================================================
ConVar zones_enabled, zones_punishment, admin_immunity, show_messages, show_zones;

Handle ZonesArray = null;
TopMenu hTopMenu;
...
float FirstZoneVector[MAXPLAYERS + 1][3], SecondZoneVector[MAXPLAYERS + 1][3];

// ====[ GLOBALS ]===========================================================
bool bLate;
char map[64], PREFIX[32];
EngineVersion CurrentVersion;
Handle OnEnteredProtectedZone, OnLeftProtectedZone;
int m_hMyWeapons, m_flNextPrimaryAttack, m_flNextSecondaryAttack, MAX_WEAPONS, LaserMaterial, HaloMaterial, GlowSprite, TeamZones[TEAM_SIZE], TeamColors[TEAM_SIZE][4];
...
public void OnPluginStart()
{
	...
	zones_enabled    = CreateConVar("sm_zones_enable",         "1", "Whether or not enable Zones plugin", FCVAR_NONE, true, 0.0, true, 1.0);
	zones_punishment = CreateConVar("sm_zones_punishment",     "6", "Determines how plugin should handle players who entered a zone (by default) \n1 = Announce in chat\n2 = Bounce back\n3 = Slay player\n4 = Dont allow to shoot\n5 = Allow only melee weapon\n6 = Custom punishment", FCVAR_NONE, true, 1.0, true, 6.0);
	admin_immunity   = CreateConVar("sm_zones_admin_immunity", "0", "Whether or not allow admins to across zones without any punishments and notificaions", FCVAR_NONE, true, 0.0, true, 1.0);
	show_messages    = CreateConVar("sm_zones_show_messages",  "1", "Whether or not show messages in chat to player that entered protected zone", FCVAR_NONE, true, 0.0, true, 1.0);
	show_zones       = CreateConVar("sm_zones_show",           "0", "Whether or not show the zones on a map all the times", FCVAR_NONE, true, 0.0, true, 1.0);
...
public int Native_Teleport(Handle plugin, int numParams)
{  
	char name[64], namezone[64];
	float posA[3], posB[3];
	
	GetNativeString(1, name, 64);
	bool sensitive = GetNativeCell(2);
	
	int size = GetArraySize(ZonesArray);
	if (size > 0)
	{
		for (int i = 0; i < size; ++i)
		{
			GetArrayString(GetArrayCell(ZonesArray, i), ZONE_NAME, namezone, 64);
			if (StrContains(name, namezone, sensitive) == 0)
			{
				GetArrayArray(GetArrayCell(ZonesArray, i),  FIRST_VECTOR,  posA, VECTORS_SIZE);
				GetArrayArray(GetArrayCell(ZonesArray, i),  SECOND_VECTOR, posB, VECTORS_SIZE);
				float ZonePos[3];
...
{  
	...
	if (size > 0)
	{
		...
		{
			GetArrayString(GetArrayCell(ZonesArray, i), ZONE_NAME, namezone, 64);
			if (StrContains(name, namezone, false) == 0)
			{
				GetArrayArray(GetArrayCell(ZonesArray, i),  FIRST_VECTOR,  posA, VECTORS_SIZE);
				GetArrayArray(GetArrayCell(ZonesArray, i),  SECOND_VECTOR, posB, VECTORS_SIZE);
				float origin[3];
				float fNull[3] = {0.0, 0.0, 0.0};
				GetMiddleOfABox(posA, posB, origin);
				if (target > 0 && IsClientInGame(target))
					TeleportEntity(target, origin, NULL_VECTOR, fNull);
			}
		}
	}
}*/
...
public void OnAdminMenuReady(Handle aTopMenu)
{
	...
	if (server_commands != INVALID_TOPMENUOBJECT)
		hTopMenu.AddItem("sm_zones", AdminMenu_Zones, server_commands, "sm_zones_immunity", ADMFLAG_CONFIG);
}
...
public void OnMapStart()
{
	...
	if (gameConfig != null)
	{
		...
		if (GameConfGetKeyValue(gameConfig, "SpriteBeam", buffer, sizeof(buffer)) && buffer[0])
		{
			...
			LogMessage("\"SpriteBeam\" material for this game is missing! Ask SourceMod developers to add one in funcommands gamedata.");
		...
		if (GameConfGetKeyValue(gameConfig, "SpriteHalo", buffer, sizeof(buffer)) && buffer[0])
		{
			...
			LogMessage("\"SpriteHalo\" material for this game is missing! Ask SourceMod developers to add one in funcommands gamedata.");
		...
		// If GlowSprite is missing for this game, log about
			...
			LogMessage("\"GlowSprite\" material for this game is missing! Ask SourceMod developers to add one in funcommands gamedata.");
	...
	if (bLate)
	{
		...
		/* OnRoundStart(null, NULL_STRING, false); */
		CreateTimer(1.0, Timer_OnRoundStart, _)
...
public Action Timer_OnRoundStart(Handle timer)
{
	// Does plugin is enabled?
	if (zones_enabled.BoolValue)
	{
		char class[MAX_ZONE_LENGTH];
		int zone, z;
		...
		while ((zone = FindEntityByClassname(zone, "trigger_multiple")) != INIT)
		{
			...
			if (IsValidEdict(zone) && GetEntPropString(zone, Prop_Data, "m_iName", class, sizeof(class)) && strncmp(class, "sm_zone", 7) == 0) // Compare first 7 characters
 ...
 * -------------------------------------------------------------------------- */
public Action OnPlayerEvents(Event event, const char[] name, bool dontBroadcast)
{
	// When player dies or respawns, allow player to use all weapons again
	WeaponPunishment[GetClientOfUserId(event.GetInt("userid"))] = false;
}
 ...
 * -------------------------------------------------------------------------- */
public int OnTouch(const char[] output, int caller, int activator, float delay)
{
	...
	if (zones_enabled.BoolValue)
	{
		if (1 <= activator <= MaxClients)
		{
			if (IsClientInGame(activator))
			{
				...
				if (admin_immunity.BoolValue && CheckCommandAccess(activator, "sm_zones_immunity", ADMFLAG_CONFIG, true))
					return;

				// Get the name of a zone
				char targetname[MAX_ZONE_LENGTH+9], ZoneName[MAX_ZONE_LENGTH];
				int i;
				...
				int punishment = INIT;
				int real_punishment = zones_punishment.IntValue;
				...
				bool messages   = show_messages.BoolValue;
				...
				for (i = 0; i < GetArraySize(ZonesArray); i++)
				{
					...
					if (StrEqual(ZoneName, targetname[8], false))
					{
						...
						if (team  != CS_TEAM_NONE && GetClientTeam(activator) != team)
							return; // If team doesnt match, skip punishments
					}
				}
				...
				switch (real_punishment)
				{
					...
					case SLAY:
					{
						...
						if (StartTouch)
						{
							...
							if (CurrentVersion == Engine_DODS)
							{
								...
								ForcePlayerSuicide(activator); // For other games use ForcePlayerSuicide
						}
					}
					case NOSHOOT:
					{
					...
					case MELEE: // Only allow the usage of the melee weapons
					{
						if (StartTouch)
						{
							...
							if (IsTF2)
								TF2_AddCondition(activator, view_as<TFCond>(TFCond_RestrictToMelee), 999.9); // For TF2 set existed condition to dont allow player to go away from melee weapon
							else
								WeaponPunishment[activator] = true; // Set boolean for weapon usage in other games
							if (messages)
...
public Action OnClientSayCommand(int client, const char[] command, const char[] text)
{
	...
	else if (RenamesZone[client])
	{
		...
		do
		{
			...
			kv.GetString("zone_ident", buffer, sizeof(buffer));
			if (StrEqual(buffer, OldZoneName, false))
			{
				// Write the new name in config
				kv.SetString("zone_ident", text);
				break;
			}
		}
 ...
 * -------------------------------------------------------------------------- */
void ShowZonesMainMenu(int client)
{
 ...
 * -------------------------------------------------------------------------- */
public int Menu_Zones(Menu menu, MenuAction action, int client, int param)
{
	if (action == MenuAction_Select)
	{
		...
		else if (StrEqual(info, "active_zones", false)) // No, maybe that was an 'Active zones' ?
			ShowActiveZonesMenu(client);
		else if (StrEqual(info, "activate_zones", false)) // Nope, that was 'Activate zones' item
			ShowDiactivatedZonesMenu(client);
		else if (StrEqual(info, "diactivate_zones", false)) // If not - then its a lates one, I believe
			ShowActivatedZonesMenu(client); // Diactivate zones
	}
	else if (action == MenuAction_End)
		delete menu; // Close menu handle on menu ending
}
 ...
 * -------------------------------------------------------------------------- */
void ShowActiveZonesMenu(int client)
{
	Menu menu = new Menu(Menu_ActiveZones);

	// Set menu title
	menu.SetTitle("%T ", "Active Zones", client);

	char name[PLATFORM_MAX_PATH], strnum[8];
 ...
 * -------------------------------------------------------------------------- */
public int Menu_ActiveZones(Menu menu, MenuAction action, int client, int param)
{
	switch (action)
	{
 ...
 * -------------------------------------------------------------------------- */
void ShowActivatedZonesMenu(int client)
{
	Menu menu = new Menu(Menu_ActivatedZones);
	menu.SetTitle("%T ", "Diactivated Zones", client);
	...
	while ((zone = FindEntityByClassname(zone, "trigger_multiple")) != INIT)
	{
		...
		&& strncmp(class, "sm_zone", 7) == 0)
			menu.AddItem(class[8], class[8]); // Set menu title and item info same as m_iName without sm_zone prefix
	}
 ...
 * -------------------------------------------------------------------------- */
public int Menu_ActivatedZones(Menu menu, MenuAction action, int client, int param)
{
	switch (action)
	{
		...
		{
			if (param == MenuCancel_ExitBack)
				ShowZonesMainMenu(client); // Show zones main menu then
		}
		case MenuAction_End:
			delete menu;
	}
 ...
 * -------------------------------------------------------------------------- */
void ShowDiactivatedZonesMenu(int client)
{
	Menu menu = new Menu(Menu_DiactivatedZones);
	menu.SetTitle("%T ", "Activated Zones", client);
	...
	while ((zone = FindEntityByClassname(zone, "trigger_multiple")) != INIT)
	{
		...
		&& strncmp(class, "sm_zone", 7) == 0) // Does name contains 'sm_zone' prefix?
			menu.AddItem(class[8], class[8]); // Add every disabled zone into diactivated menu
	}
 ...
 * -------------------------------------------------------------------------- */
public int Menu_DiactivatedZones(Menu menu, MenuAction action, int client, int param)
{
	switch (action)
	{
		case MenuAction_Select:
 ...
 * -------------------------------------------------------------------------- */
void ShowZoneOptionsMenu(int client)
{
	// Make sure player is not editing any other zone at this moment
	if (EditingZone[client] != INIT)
	{
		char ZoneName[MAX_ZONE_LENGTH], translation[128], buffer[128];
		int team;
 ...
 * -------------------------------------------------------------------------- */
public int Menu_ZoneOptions(Menu menu, MenuAction action, int client, int param)
{
	...
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			char config[PLATFORM_MAX_PATH], ZoneName[MAX_ZONE_LENGTH], info[11];
			float vec1[3], vec2[3];
			int team;
			menu.GetItem(param, info, sizeof(info));
			BuildPath(Path_SM, config, sizeof(config), "data/zones/%s.cfg", map);
			...
			if (StrEqual(info, "teleport", false))
			{
				...
				TeleportEntity(client, origin, NULL_VECTOR, fNull);
			...
			else if (StrEqual(info, "team", false))
			{
				...
				if (team > CS_TEAM_CT)
					...
					team = CS_TEAM_T; // Same here, but set lowerbounds to first available team
				...
				do
				{
					...
					if (StrEqual(buffer, ZoneName, false))
					{
						...
						if (team == CS_TEAM_NONE)
							kv.DeleteKey("restrict_team");
						else
							kv.SetNum("restrict_team", team);
						break;
					}
				}
				while (kv.GotoNextKey());
			...
			else if (StrEqual(info, "punishment", false))
			{
				...
				if (real_punishment > CUSTOM)
					real_punishment = INIT; // Re-init punishments on overbounds
				...
				do
				{
					...
					if (StrEqual(buffer, ZoneName, false))
					{
						...
						if (real_punishment == INIT)
							kv.DeleteKey("punishment");
						else
							kv.SetNum("punishment", real_punishment);
						break;
					}
				}
				while (kv.GotoNextKey());
			...
			else if (StrEqual(info, "vec1", false) || StrEqual(info, "vec2", false))
			{
				if (StrEqual(info, "vec1", false))
					...
					EditingVector[client] = SECOND_VECTOR;
			...
			else if (StrEqual(info, "delete", false))
			{
				...
				// Send panel
				panel.Send(client, Panel_Confirmation, MENU_TIME_FOREVER);
		...
		case MenuAction_Cancel:
		{
			...
			if (param == MenuCancel_ExitBack)
				ShowActiveZonesMenu(client); // Show active zones menu
		}
		case MenuAction_End:
			delete menu;
	}
 ...
 * -------------------------------------------------------------------------- */
public int Menu_ZoneVectorEdit(Menu menu, MenuAction action, int client, int param)
{
	switch (action)
	{
		case MenuAction_Select:
			...
			if (StrEqual(info, "save", false))
			{
				...
				do
				{
					...
					if (StrEqual(buffer, ZoneName, false))
					{
						...
						kv.SetVector("coordinates 1", FirstZoneVector[client]);
						kv.SetVector("coordinates 2", SecondZoneVector[client]);
						break;
					}
				}
				while (kv.GotoNextKey()); // Until config is ended

				kv.Rewind();
				kv.ExportToFile(config);
				delete kv;
			}
			else if (StrEqual(info, "ax", false)) // Add X
			{
				// Add to the x axis
				if (EditingVector[client] == FIRST_VECTOR)
					FirstZoneVector[client][0] += 5.0; // Move zone for 5 units on every selection
				else
					SecondZoneVector[client][0] += 5.0;
			}
		...
		case MenuAction_Cancel: 
		{
			...
			if (param == MenuCancel_ExitBack)
				ShowZoneOptionsMenu(client); // Redraw zone options menu
			else
				EditingZone[client] = INIT; // When player just pressed Exit button, make sure player is not editing any zone anymore
		}
		case MenuAction_End:
			delete menu;
	}
 ...
 * -------------------------------------------------------------------------- */
void ShowSaveZoneMenu(int client, const char[] name)
{
	char translation[128];

	// Confirm the new zone after naming
	Menu menu = new Menu(Menu_SaveZone);
	menu.SetTitle("%T", "Adding Zone", client);

	// Add 2 options to menu - Save & Discard
	Format(translation, sizeof(translation), "%T", "Save", client);
	menu.AddItem(name, translation);
	Format(translation, sizeof(translation), "%T", "Discard", client);
	menu.AddItem("discard", translation);
 ...
 * -------------------------------------------------------------------------- */
public int Menu_SaveZone(Menu menu, MenuAction action, int client, int param)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			else // Save the new zone, because any other item is selected
			{
				...
				char buffer[MAX_ZONE_LENGTH], strnum[8];
				int temp;
				if (kv.GotoFirstSubKey())
				{
					do
					{
						...
						if (temp >= number)
							number = ++temp; // Set another increased number for zone in config
				...
				// Jump to zone number
				kv.JumpToKey(strnum, true);

				// Set name and coordinates
				kv.SetString("zone_ident",    info);
				kv.SetVector("coordinates 1", FirstZoneVector[client]);
				kv.SetVector("coordinates 2", SecondZoneVector[client]);
		...
		case MenuAction_Cancel:
		{
			...
			if (param == MenuCancel_ExitBack)
				ShowActiveZonesMenu(client); // If player pressed back button, show active zones menu (again)
		}
		case MenuAction_End:
			delete menu;
	}
 ...
 * -------------------------------------------------------------------------- */
public int Panel_Confirmation(Menu menu, MenuAction action, int client, int param)
{
		...
		for (int i; i < GetArraySize(ZonesArray); i++)
		{
			...
			for (client = 1; client <= MaxClients; client++)
			{
				if (IsClientInGame(client))
				{
					...
					else if (GetConVarBool(show_zones) && (team == CS_TEAM_NONE || (GetClientTeam(client) == team))) // Otherwise always show zones if plugin is set it to true
...
void ParseZoneConfig()
{
	...
	if (FileExists(config))
	{
		...
		do
		{
			...
			kv.GetVector("coordinates 1", vector);
			PushArrayArray(TempArray, vector, VECTORS_SIZE);

			// Second coordinations
			kv.GetVector("coordinates 2", vector);
			PushArrayArray(TempArray, vector, VECTORS_SIZE);

			// Get the team restrictions
			int team = kv.GetNum("restrict_team", CS_TEAM_NONE);
			...
			// Get the punishments
			real_punishment = kv.GetNum("punishment", INIT);
...
void ActivateZone(const char[] text)
{
	...
	while ((zone = FindEntityByClassname(zone, "trigger_multiple")) != INIT)
	{
		if (IsValidEdict(zone) && GetEntPropString(zone, Prop_Data, "m_iName", class, sizeof(class))
...
void DiactivateZone(const char[] text)
{
	...
	while ((zone = FindEntityByClassname(zone, "trigger_multiple")) != INIT)
	{
		if (IsValidEdict(zone) && GetEntPropString(zone, Prop_Data, "m_iName", class, sizeof(class)) && StrEqual(class[8], text, false))
		{
...
void SpawnZone(int zoneIndex)
{
	...
	float middle[3], m_vecMins[3], m_vecMaxs[3];
...
{
	char ZoneName[MAX_ZONE_LENGTH], class[MAX_ZONE_LENGTH+9];
	int zone;
	...
	while ((zone = FindEntityByClassname(zone, "trigger_multiple")) != INIT)
	{
		if (IsValidEdict(zone) && GetEntPropString(zone, Prop_Data, "m_iName", class, sizeof(class)) // Get m_iName datamap
 ...
 * @noreturn
  * -------------------------------------------------------------------------- */
void TE_SendBeamBoxToClient(int client, const float upc[3], const float btc[3], int ModelIndex, int HaloIndex, int StartFrame, int FrameRate, const float Life, const float Width, const float EndWidth, int FadeLength, const float Amplitude, const Color[4], int Speed)
{
