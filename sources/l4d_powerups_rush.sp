...
float g_fl_AutoS = 0.666666;
float g_fl_AutoI = 0.4;
float g_fl_AutoE = 0.675;
float g_fl_SpasS = 0.5;
float g_fl_SpasI = 0.375;
float g_fl_SpasE = 0.699999;
float g_fl_PumpS = 0.5;
float g_fl_PumpI = 0.5;
float g_fl_PumpE = 0.6;
/* ***************************************************************************/
...
public void OnPluginStart()
{
	...
	powerups_duration = CreateConVar("l4d_powerups_duration", "60", "How long should the duration of the boosts last?", FCVAR_NONE|FCVAR_NOTIFY, true, 1.0);
...
//	pills_luck = CreateConVar("l4d_powerups_pills_luck", "1", "The luck factor for swallowing pills that will grant the boost(s). (1 = 1/1  2 = 1/2  3 = 1/3  4 = 1/4  etc.)", FCVAR_NONE|FCVAR_NOTIFY, true, 1.0);
	
	g_h_reload_rate = CreateConVar("l4d_powerups_weaponreload_rate", "0.9",	"The interval incurred by reloading is multiplied by this value (clamped between 0.2 < 0.9)", FCVAR_NONE|FCVAR_NOTIFY, true, 0.2, true, 1.0);
	HookConVarChange(g_h_reload_rate, Convar_Reload);
	g_fl_reload_rate = 0.5714;
		
	g_h_melee_rate = CreateConVar("l4d_powerups_weaponmelee_rate", "0.5", "The interval for swinging melee weapon (clamped between 0.3 < 0.9)",	FCVAR_NONE|FCVAR_NOTIFY, true, 0.3, true, 0.9);
		
	g_hDT_rate = CreateConVar("l4d_powerups_weaponfiring_rate", "1.0" , "The interval between bullets fired is multiplied by this value. WARNING: a short enough interval will make some guns firing accuracy distorted (clamped between 0.2 < 0.9)", FCVAR_NONE|FCVAR_NOTIFY, true, 0.2, true, 1.0);
...
/* ***************************************************************************/
public void Convar_Reload (Handle convar, const char[] oldValue, const char[] newValue)
{
	float flF = StringToFloat(newValue);
	if (flF < 0.02) flF = 0.02;
	else if (flF > 0.9) flF = 0.9;
	g_fl_reload_rate = flF;
}

public void Convar_DT (Handle convar, const char[] oldValue, const char[] newValue)
{
	float flF = StringToFloat(newValue);
	if (flF < 0.02) flF = 0.02;
	else if (flF > 0.9) flF = 0.9;
...
int MagStart(int iEntid, int client)
{
	#if DEBUG
