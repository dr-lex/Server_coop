...
Handle sdkShoveSurv = null;
Handle sdkShoveInf = null;
...
public void OnPluginStart()
{
	...
	//Cvars
	CreateConVar("l4d2_custom_commands_version", GETVERSION, "Version of Custom Admin Commands Plugin", FCVAR_NONE|FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	g_cvarRadius = CreateConVar("l4d2_custom_commands_explosion_radius", "350", "Radius for the Create Explosion's command explosion", FCVAR_NONE);
	g_cvarPower = CreateConVar("l4d2_custom_commands_explosion_power", "350", "Power of the Create Explosion's command explosion", FCVAR_NONE);
	g_cvarDuration = CreateConVar("l4d2_custom_commands_explosion_duration", "15", "Duration of the Create Explosion's command explosion fire trace", FCVAR_NONE);
	g_cvarRainDur = CreateConVar("l4d2_custom_commands_rain_duration", "10", "Time out for the gnome's rain or l4d1 survivors rain", FCVAR_NONE);
	g_cvarRainRadius = CreateConVar("l4d2_custom_commands_rain_radius", "300", "Maximum radius of the gnome rain or l4d1 rain. Will also affect the air strike radius", FCVAR_NONE);
	g_cvarLog = CreateConVar("l4d2_custom_commands_log", "1", "Log admin actions when they use a command? [1: Yes 0: No]", FCVAR_NONE);
	g_cvarAddType = CreateConVar("l4d2_custom_commands_menutype", "1", "How should the commands be added to the menu? 0: Create new category 1: Add to default categories", FCVAR_NONE);
...
stock int BuildPlayerMenu(int client)
{
...
stock int BuildGeneralMenu(int client)
{
...
stock int BuildServerMenu(int client)
{
...
public int MenuHandler_PlayerMenu(Menu menu, MenuAction action, int param1, int param2)
{
	...
	{
		if (param2 == MenuCancel_ExitBack && GetAdminTopMenu() != null)
		{
...
public int MenuHandler_GeneralMenu(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		switch(param2)
...
/*public int MenuItem_WipeBody(Handle topmenu, TopMenuAction action, TopMenuObject object_id, int param, char[] buffer, int maxlength)
{
	if (action == TopMenuAction_DisplayOption)
	{
...
*/
//---------------------------------Display menus---------------------------------------
int DisplayVomitPlayerMenu(int client)
{
	Menu menu2 = CreateMenu(MenuHandler_VomitPlayer);
...
int DisplayTeleportPlayerMenu(int client)
{
	Menu menu2 = CreateMenu(MenuHandler_TeleportPlayer);
	SetMenuTitle(menu2, "Select Player:");
	SetMenuExitBackButton(menu2, true);
	AddTargetsToMenu2(menu2, client, COMMAND_FILTER_CONNECTED);
	DisplayMenu(menu2, client, MENU_TIME_FOREVER);
...
int DisplayChargePlayerMenu(int client)
{
	Menu menu2 = CreateMenu(MenuHandler_ChargePlayer);
	SetMenuTitle(menu2, "Select Player:");
	SetMenuExitBackButton(menu2, true);
	AddTargetsToMenu2(menu2, client, COMMAND_FILTER_CONNECTED);
	DisplayMenu(menu2, client, MENU_TIME_FOREVER);
...
int DisplayGodModeMenu(int client)
{
	Menu menu2 = CreateMenu(MenuHandler_GodMode);
	SetMenuTitle(menu2, "Select Player:");
	SetMenuExitBackButton(menu2, true);
	AddTargetsToMenu2(menu2, client, COMMAND_FILTER_CONNECTED);
	DisplayMenu(menu2, client, MENU_TIME_FOREVER);
...
int DisplayIncapPlayerMenu(int client)
{
	Menu menu3 = CreateMenu(MenuHandler_IncapPlayer);
...
int DisplaySpeedPlayerMenu(int client)
{
	Menu menu4 = CreateMenu(MenuSubHandler_SpeedPlayer);
...
int DisplaySetHpPlayerMenu(int client)
{
	Menu menu5 = CreateMenu(MenuSubHandler_SetHpPlayer);
...
int DisplayChangeHpMenu(int client)
{
	Menu menu5 = CreateMenu(MenuSubHandler_ChangeHp);
	SetMenuTitle(menu5, "Select Player:");
	SetMenuExitBackButton(menu5, true);
	AddTargetsToMenu2(menu5, client, COMMAND_FILTER_CONNECTED);
	DisplayMenu(menu5, client, MENU_TIME_FOREVER);
...
int DisplayColorPlayerMenu(int client)
{
	Menu menu6 = CreateMenu(MenuSubHandler_ColorPlayer);
...
int DisplayCreateExplosionMenu(int client)
{
	Menu menu7 = CreateMenu(MenuHandler_CreateExplosion);
...
int DisplayScalePlayerMenu(int client)
{
	Menu menu8 = CreateMenu(MenuSubHandler_ScalePlayer);
...
int DisplayShakePlayerMenu(int client)
{
	Menu menu8 = CreateMenu(MenuSubHandler_ShakePlayer);
	SetMenuTitle(menu8, "Select Player:");
	SetMenuExitBackButton(menu8, true);
	AddTargetsToMenu2(menu8, client, COMMAND_FILTER_CONNECTED);
	DisplayMenu(menu8, client, MENU_TIME_FOREVER);
...
int DisplayBugPlayerMenu(int client)
{
	Menu menu9 = CreateMenu(MenuHandler_BugPlayer);
...
int DisplayDontRushMenu(int client)
{
	Menu menu10 = CreateMenu(MenuHandler_DontRush);
...
int DisplayAirstrikeMenu(int client)
{
	Menu menu11 = CreateMenu(MenuHandler_Airstrike);
...
int DisplaySpeedValueMenu(int client)
{
	Menu menu2a = CreateMenu(MenuHandler_SpeedPlayer);
...
int DisplaySetHpValueMenu(int client)
{
	Menu menu2b = CreateMenu(MenuHandler_SetHpPlayer);
...
int DisplayColorValueMenu(int client)
{
	Menu menu2c = CreateMenu(MenuHandler_ColorPlayer);
...
int DisplayScaleValueMenu(int client)
{
	Menu menu2a = CreateMenu(MenuHandler_ScalePlayer);
...
int DisplayShakeValueMenu(int client)
{
	Menu menu2a = CreateMenu(MenuHandler_ShakePlayer);
...
int DisplayChangeHpStyleMenu(int client)
{
	Menu menu2a = CreateMenu(MenuHandler_ChangeHpPlayer);
...
//-------------------------------Do action------------------------------------
public int MenuHandler_VomitPlayer(Menu menu2, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_End)
	{
		CloseHandle(menu2);
...
public int MenuHandler_ChargePlayer(Menu menu2, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_End)
	{
		CloseHandle(menu2);
...
public int MenuHandler_BugPlayer(Menu menu9, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_End)
	{
		CloseHandle(menu9);
...
int CreateExplosion(float carPos[3])
{
	char sRadius[256];
	char sPower[256];
...
{
	ResetPack(pack);
	any particle = ReadPackCell(pack);
	any hurt = ReadPackCell(pack);
	CloseHandle(pack);
...
stock int FlingPlayer(int target, float vector[3], int attacker, float stunTime = 3.0)
{
...
/*int LaunchMissile(int target, int sender)
{
...
stock int SwitchHealth(int target, int sender, int type)
{
	if (target > 0 && IsValidEntity(target) && IsClientInGame(target) && IsPlayerAlive(target))
	{
		...
		else if(type == 2)
		{
			int iTempHealth = GetClientTempHealth(target);
			int iPermHealth = GetClientHealth(target);
			float flTotal = view_as<float>(iTempHealth+iPermHealth);
			SetEntityHealth(target, 1);
			RemoveTempHealth(target);
...
stock int WeaponRain(char[] weapon, int sender)
{
	char item[64];
...
stock int StartGnomeRain(int client)
{
	g_bGnomeRain = true;
...
stock int StartL4dRain(int client)
{
	g_bGnomeRain = true;
	CreateTimer(GetConVarFloat(g_cvarRainDur), timerRainTimeout, _, TIMER_FLAG_NO_MAPCHANGE);
...
stock int GodMode(int target, int sender)
{
	if (GetClientTeam(target) == 1)
	{
...
stock int CheatCommand(int client, const char[] command, const char[] arguments)
{
	if (!client) return;
...
stock int InstructorHint(char[] content)
{	
	for(int i=1; i<=MaxClients; i++)
	{
...
stock int CreateParticle(int client, char[] Particle_Name, bool Parent, float duration)
{
	float pos[3];
	char sName[64], sTargetName[64];
	int Particle = CreateEntityByName("info_particle_system");
...
stock int IgnitePlayer(int client, float duration)
{
...
stock int PrecacheParticle(char[] ParticleName)
{
	int Particle = CreateEntityByName("info_particle_system");
...
stock int CreateAcidSpill(int iTarget, int iSender)
{
	float vecPos[3];
...
stock int SetAdrenalineEffect(int iTarget, int iSender)
{
	SDKCall(sdkAdrenaline, iTarget, 15.0);
}

stock int SetTempHealth(int iTarget, float flAmount)
{
	SDKCall(sdkSetBuffer, iTarget, flAmount);
}

stock int RevivePlayer(int iTarget, int iSender)
{
