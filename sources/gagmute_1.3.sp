...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_addbanlist",  CMD_addbanlist,  ADMFLAG_ROOT,  "");
	...
	BuildPath(Path_SM, sg_blacklist, sizeof(sg_blacklist)-1, "data/blacklist.txt");
...
void HxClientGagMuteBan2(int &client)
{
	KeyValues hGM = new KeyValues("list");

	if (hGM.ImportFromFile(sg_blacklist))
	{
		char sTeamID[24];
		GetClientAuthId(client, AuthId_Steam2, sTeamID, sizeof(sTeamID)-1);
		...
		if (hGM.JumpToKey(sTeamID))
		{
			int iBan = hGM.GetNum("ban");
			if (iBan)
			{
				KickClient(client,"[Blacklist] You are on the blacklist!");
			}
		}
	}
	delete hGM;
...
public Action CMD_addbanlist(int client, int args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "Usage: sm_bantime <STEAM_ID>");
		return Plugin_Handled;
	}
	...
	HxClientTimeBanSteam2(authid);
	int i = 1;
	while (i <= MaxClients)
	{
		if (IsClientInGame(i))
		{
			HxClientGagMuteBan2(i);
		}
		i += 1;
	}
	return Plugin_Handled;
...
int HxClientTimeBanSteam2(char[] steam_id)
{
	KeyValues hGM = new KeyValues("list");
	hGM.ImportFromFile(sg_blacklist);
