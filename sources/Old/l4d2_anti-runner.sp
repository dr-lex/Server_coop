...
void ClientKick()
{
	...
	ForcePlayerSuicide(ig_keyman);
	ServerCommand("exec hardmod/antifarm.cfg");
...
public Action Event_Player_Use(Event event, const char[] name, bool dontBroadcast)
{
	...
	if ((ig_SafetyLock == LOCK) && (Entity == ig_door) && IsValidEntity(Entity))
	{
		...
		{
			PrintHintTextToAll("Keyman (%N).", ig_keyman);
			PrintHintText(ig_keyman, "Keyman (%N).", ig_keyman);
		}
	}
	return Plugin_Continue;
}
