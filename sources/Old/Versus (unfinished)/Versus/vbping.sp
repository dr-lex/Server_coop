...
-- 1.4 (7/15/10)
 ...
 1.4.4 (29-Jan-2022) (Dragokas)
 - Added Hungarian translation (thanks to @KasperH)
 - Allowed to set ignore hours over a midnight, like 23 - 1.
 - Fixed invalid ConVar hook prototype.
...
public Action timer_ExpirePingDelay(Handle timer, any UserId)
{
...
public Action timer_CheckPing(Handle timer)
{
	...
	if (TimerCheck) return Plugin_Continue;
	...
	if( g_iCvarIgnoreHourMin != -1 )
	{
		...
		if( g_iCvarIgnoreHourMin > g_iCvarIgnoreHourMax )
		{
			if( hours >= g_iCvarIgnoreHourMin )
				return Plugin_Continue;
				
			if( hours <= g_iCvarIgnoreHourMax )
				return Plugin_Continue;
		}
		else {
	...
	if (Players < g_iCvarMinPlayers) return Plugin_Continue;
