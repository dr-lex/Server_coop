...
public void OnPluginStart()
{
	...
	g_hCvarDmgSurvivor = CreateConVar(	"l4d2_spitter_acid_dmg_survivor",	"2.0",			"Damage dealt to survivors (games default is 1.0 with l4d2_spitter_acid_damage 2).", CVAR_FLAGS);
	g_hCvarDmgSurvBots = CreateConVar(	"l4d2_spitter_acid_dmg_bots",		"2.0",			"Damage dealt to survivor bots (games default is 1.0 with l4d2_spitter_acid_damage 2).", CVAR_FLAGS);
...
public Action OnTakeDamage(int entity, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	if (damagetype == 263168 || damagetype == 265216) // 265216 at end of entity life when fading out
	{
		...
		if (entity > MaxClients)
		{
			...
			if (g_iCvarEffects & (1<<0))
			{
				DisplayParticle(entity, PARTICLE_SPIT);
			}
