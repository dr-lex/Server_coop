...
PrepareAllSDKCalls()
{
	...
	DebugPrintToAll("CDirector::RestartScenarioFromVote Call prepped");
	if(!PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "CDirector::RestartScenarioFromVote"))
	{
		LogError("Could not load the CDirector::RestartScenarioFromVote signature");
	}
	PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);
	DebugPrintToAll("CDirector::RestartScenarioFromVote Signature prepped");
	fRestart = EndPrepSDKCall();
	if(fRestart == INVALID_HANDLE)
	{
		LogError("Could not prep the CDirector::RestartScenarioFromVote function");
	}

	StartPrepSDKCall(SDKCall_Raw);
	DebugPrintToAll("CDirector::SwapTeams Call prepped");
	if(!PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "CDirector::SwapTeams"))
	{
		LogError("Could not load the CDirector::SwapTeams signature");
	}
	DebugPrintToAll("CDirector::SwapTeams Signature prepped");
	fSwapTeams = EndPrepSDKCall();
	if(fSwapTeams == INVALID_HANDLE)
	{
		ThrowError("Could not prep the CDirector::SwapTeams function");
	}

	StartPrepSDKCall(SDKCall_Raw);
	DebugPrintToAll("CDirector::AreTeamsFlipped Call prepped");
	if(!PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "CDirector::AreTeamsFlipped"))
	{
		LogError("Could not load the CDirector::AreTeamsFlipped signature");
	}
	PrepSDKCall_SetReturnInfo(SDKType_Bool, SDKPass_Plain);
	DebugPrintToAll("CDirector::AreTeamsFlipped Signature prepped");
	fAreTeamsFlipped = EndPrepSDKCall();
	if(fAreTeamsFlipped == INVALID_HANDLE)
	{
		ThrowError("Could not prep the CDirector::AreTeamsFlipped function");
	}
}
...
{
	DebugPrintToAll("About to SDKCall CDirector::SwapTeams. g_pDirector=%x", g_pDirector);
...
{
	DebugPrintToAll("About to SDKCall CDirector::RestartScenarioFromVote. g_pDirector=%x", g_pDirector);
