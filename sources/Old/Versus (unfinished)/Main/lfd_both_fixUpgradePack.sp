...
void PlayDenySound(int client)
{
	if (cvarDeniedSoundValue)
	{
		...
		if (currentTime > lastSoundTime[client] + 2.0)
		{
			...
			{
				EmitSoundToClient(client, sDENY_SOUND, client, 3);
			}
...
void CheckKillPackage(int entity)
{
	...
	if (bDeleteEntity[entity] == false)
	{
		...
		if (cvarClearUpgradeTimeValue > 0.0)
		{
			CreateTimer(cvarClearUpgradeTimeValue, Timer_RemoveEntity, EntIndexToEntRef(entity), TIMER_FLAG_NO_MAPCHANGE);
		}
	}
}
